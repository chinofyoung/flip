    ...
    return (
        <div className="space-y-4">
            <div className="flex items-center justify-between bg-black/40 px-4 py-2 rounded-xl border border-white/5 backdrop-blur-sm">
                ...
                <span className="text-[10px] font-bold text-gold uppercase tracking-widest hidden sm:inline-block">
                    ...
                    {sortedPlayers.map((player, index) => {
                        ...
                        return (
                            <motion.div
                                key={`grid-${player.uid}`}
                                layout
                                className={`relative flex flex-col items-center p-3 rounded-2xl transition-all border aspect-square justify-center ${isCurrentUser
                                    ? "bg-gold/5 border-gold/20 shadow-lg shadow-gold/5"
                                    : "bg-surface/50 border-white/5 shadow-md"
                                    }`}
                            >
                                {/* Rank */}
                                <div className="absolute top-2 left-2 text-[9px] font-black text-muted/20">
                                    {String(index + 1).padStart(2, '0')}
                                </div>
                                ...
                                {/* Avatar */}
                                <div className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center shrink-0 overflow-hidden shadow-inner mb-2">
                                    {player.photoURL ? (
                                        <img
                                            src={player.photoURL}
                                            alt=""
                                ...
                                <span className={`text-[10px] font-black uppercase tracking-tight truncate max-w-full ${isCurrentUser ? "text-gold" : "text-foreground"}`}>
                                ...
                                {hand && (
                                    <div
                                        className={`absolute inset-0 rounded-2xl border-2 pointer-events-none ${hand.status === "busted"
                                            ? "border-red-500/20"
                                            : hand.status === "stayed"
